...
if WOW_PROJECT_ID >= WOW_PROJECT_CATACLYSM_CLASSIC then
    ...
    function TotemTimers.GetSpecialization()
        local pointsSpent = 0
        for i=1,3 do
            -- TBC (including Anniversary) and Classic use position 5, retail uses position 3
            local isTBCorClassic = (WOW_PROJECT_ID == WOW_PROJECT_CLASSIC) or (WOW_PROJECT_ID == 5) or (WOW_PROJECT_BURNING_CRUSADE_CLASSIC and WOW_PROJECT_ID == WOW_PROJECT_BURNING_CRUSADE_CLASSIC)
            local points = select(isTBCorClassic and 5 or 3, GetTalentTabInfo(i))

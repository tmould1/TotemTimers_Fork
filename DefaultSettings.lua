...
-- Initialize ActiveProfile with defaults first, will be updated later with saved settings
-- This prevents nil errors when GUI loads before PLAYER_ENTERING_WORLD
TotemTimers.ActiveProfile = {}
for k, v in pairs(TotemTimers.DefaultProfile) do
    if type(v) == "table" then
        TotemTimers.ActiveProfile[k] = {}
        for k2, v2 in pairs(v) do
            if type(v2) == "table" then
                TotemTimers.ActiveProfile[k][k2] = {}
                for k3, v3 in pairs(v2) do
                    TotemTimers.ActiveProfile[k][k2][k3] = v3
                end
            else
                TotemTimers.ActiveProfile[k][k2] = v2
            end
        end
    else
        TotemTimers.ActiveProfile[k] = v
    end
end

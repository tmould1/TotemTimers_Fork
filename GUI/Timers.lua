...
TotemTimers.options.args.timers = {
    ...
    args = {
        ...
        timer1 = {
            ...
            get = function(info)
                local order = TotemTimers.ActiveProfile.Order or {1,2,3,4}
                return order[1]
            end,
        },
        timer2 = {
        ...
        stoppulsedemo = {
            ...
            name = L["Demo"],
            func = function()
                if XiTimers and XiTimers.timers and XiTimers.timers[1] and XiTimers.timers[1].animation then
...
for i = 1, 4 do
    TotemTimers.options.args.timers.args['macro' .. i] = {
        ...
        width = 0.8,
        set = function(info, val)
            if not TotemTimers.ActiveProfile.IncludeInMacro then
                TotemTimers.ActiveProfile.IncludeInMacro = {[1] = true, [2] = true, [3] = true, [4] = true}
            end
            TotemTimers.ActiveProfile.IncludeInMacro[info.arg] = val
            TotemTimers.UpdateMacro()
        end,
        get = function(info)
            local inc = TotemTimers.ActiveProfile.IncludeInMacro or {[1] = true, [2] = true, [3] = true, [4] = true}
            return inc[info.arg]
        end,
    }
end
